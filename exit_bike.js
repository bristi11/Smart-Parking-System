    ...
    try{
        const slot_b = await new Promise((resolve, reject) => {
            ...
            if (resl.length > 0) {
                ...
                console.log(slot_no);
                var query=`update slot_bike set status=0 where slot_no='${slot_no}';`        
                con.query(query,(err,resl)=>{
                        if (err) throw err;
          ...
          if (card_type === 'regular') {
            let query=`select timediff(departure_time,arrival_time) as timedifference from booking_bike where card_no='${card_no}'` ;
...
async function exit_bike(card_no,card_type){
